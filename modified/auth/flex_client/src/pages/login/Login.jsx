...
export default function Login() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      const response = await ApiService.post("accessControl/login", data);
      if (response.data.isSuccess) {
        ...
        if (response?.data.document.giveAccess) {
          ...
          setShow(true);
          seterrorShow(
            "This user does not have the access to login. Please contact with administrator."
          );
      ...
      <Container className="loginForm">
        <Row className="justify-content-center">
          <Col xxl={4} xs={12} sm={10} md={6} lg={6} xl={6}>
            <Card
              className="shadow-lg loginForm m-0 p-0"
              style={{ width: "100%", padding: "1rem" }}
            >
              <div className="companyLogo">
                <Card.Img
                  variant="top"
                  style={{ width: "2rem", alignSelf: "left" }}
                  src="/static/paapri_logo.png"
                />
                <h3
                  style={{
                    alignSelf: "left",
