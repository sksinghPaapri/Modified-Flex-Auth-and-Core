...
exports.getAll = catchAsync(async (req, res, next) => {
  ...
  const docs = await AppCenter.find({
    permissionCode: { $in: aggResults[0]?.appPermissions },
  }).sort({ name: 1 });
...
exports.getList = catchAsync(async (req, res, next) => {
  const docs = await AppCenter.find({ isInactive: false })
    .select("name permissionCode")
    .sort({ name: 1 });
  console.log("appcenter", docs);
  res.status(200).json({
    isSuccess: true,
    status: "success",
    results: docs.length,
