...
export default function TimeEntry() {
  ...
  const createDocument = (data) => {
    ...
    return ApiService.post("/timeEntry", data)
      .then((response) => {
        if (response.data.isSuccess) {
          navigate(`/${rootPath}/timeentry/list`);
        }
      })
      .catch((e) => {
  ...
  const updateDocument = (id, data) => {
    ApiService.setHeader();
    return ApiService.patch(`/timeEntry/${id}`, data)
      .then((response) => {
  ...
  const deleteDocument = () => {
    ApiService.setHeader();
    return ApiService.delete(`/timeEntry/${id}`)
      .then((response) => {
  ...
  const findOneDocument = () => {
    ApiService.setHeader();
    return ApiService.get(`/timeEntry/${id}`)
      .then((response) => {
            ...
            <Col className="p-0 ps-2">
              ...
              >
                <Breadcrumb.Item
                  ...
                  linkAs={Link}
                  linkProps={{ to: `/${rootPath}/timeentry/list` }}
                >
                  {" "}
                  <div className="breadcrum-label">TIMEENTRY</div>
                </Breadcrumb.Item>
                {isAddMode ? (
          ...
          <Row>
            <TextField
              ...
              field={{
                ...
                placeholder: "",
                required: true,
                validationMessage: "Please enter the Name name!",
              }}
              changeHandler={null}
              blurHandler={null}
            />
