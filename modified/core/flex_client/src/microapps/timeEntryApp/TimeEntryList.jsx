...
export default function TimeEntryList() {
  ...
  const findAllDocument = async () => {
    ...
    const response = await ApiService.get("timeEntry");
    console.log("timeentry", response.data.documents);
      ...
      cellRenderer: (params) => (
        <>
          <Button
            ...
            size="sm"
            as={Link}
            to={`/${rootPath}/timeentry/edit/${params.value}`}
          >
            <BsBoxArrowInUpRight />
          </Button>
        ...
        <Container fluid>
            ...
            <Col className="p-0 ps-2">
              ...
              >
                <Breadcrumb.Item active>
                  {" "}
                  <div className="breadcrum-label-active">TIMEENTRY</div>
                </Breadcrumb.Item>
          ...
          <Row style={{ marginTop: "-10px" }}>
            <Col className="p-0 ps-1">
              <Button size="sm" as={Link} to={`/${rootPath}/timeentry/add`}>
                CREATE
              </Button>{" "}
            </Col>
            <Col md="4" sm="6">
              <Row>
                <Col md="8">
                  <input
                    type="text"
                    className="openning-cash-control__amount--input"
