...
export default function ClassList() {
    ...
    const findAllDocument = async () => {
        ...
        const response = await ApiService.get(`${_config.api.relativeUrl}`);
    ...
    const columns = [
        {
            headerName: ' ', field: 'id', sortable: false, filter: false, cellRenderer: (params) =>
                <>
                    <Button style={{ minWidth: "4rem" }} size="sm" as={Link} to={`/${rootPath}/${_config.appRoutes.baseUrl}/edit/${params.value}`}><BsBoxArrowInUpRight /></Button>
                ...
                <Container fluid >
                    <Row>
                        <Col className='p-0 ps-2'>
                            <Breadcrumb style={{ fontSize: '24px', marginBottom: '0 !important' }}>
                                <Breadcrumb.Item active> <div className='breadcrum-label-active'>{_config.appName}</div></Breadcrumb.Item>
                    ...
                    <Row style={{ marginTop: '-10px' }}>
                        <Col className='p-0 ps-1'>
                            <Button size="sm" as={Link} to={`/${rootPath}/${_config.appRoutes.baseUrl}/add`}>CREATE</Button>{" "}
                        </Col>
                        <Col md="4" sm="6">
                            <Row>
