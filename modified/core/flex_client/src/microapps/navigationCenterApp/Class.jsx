...
export default function Class() {
    ...
    const createDocument = (data) => {
        ...
        return ApiService.post(`${_config.api.relativeUrl}`, data).then(response => {
    ...
    const updateDocument = (id, data) => {
        ApiService.setHeader();
        return ApiService.patch(`/${_config.api.relativeUrl}/${id}`, data).then(response => {
    ...
    const deleteDocument = () => {
        ApiService.setHeader();
        return ApiService.delete(`/${_config.api.relativeUrl}/${id}`).then(response => {
    ...
    const findOneDocument = () => {
        ApiService.setHeader();
        return ApiService.get(`/${_config.api.relativeUrl}/${id}`).then(response => {
                ...
                <Container fluid >
                    <Row>
                        <Col className='p-0 ps-2'>
                            <Breadcrumb style={{ fontSize: '24px', marginBottom: '0 !important' }}>
                                <Breadcrumb.Item className='breadcrumb-item' linkAs={Link} linkProps={{ to: `/${rootPath}/${_config.appRoutes.baseUrl}/list` }}>   <div className='breadcrum-label'>{_config.appName}</div></Breadcrumb.Item>
                    ...
                    <Row style={{ marginTop: '-10px' }}>
                        <Col className='p-0 ps-1'>
                            ...
                            <Button as={Link} to={`/${rootPath}/${_config.appRoutes.baseUrl}/list`} variant="secondary" size="sm">DISCARD</Button>
